...
function renderPagination(data, check) {
    ...
    function loadPage(targetPage) {
        if (currentMode === 'post') {
			// 페이지 이동할때마다 상세 게시글 empty();
        	getMain(targetPage, check);
        } else {
    ...
    $("#prev-page").prop("disabled", !data.hasPrevious) // 이전 페이지가 없으면 버튼 비활성화
                   ...
                   .click(() => { // 버튼 클릭시 발생하는 이벤트
                	   ...
                	   if(data.hasPrevious) {
                		    // Math.max로 음수 방지
				        	loadPage(Math.max(currentPage - 1, 0), check)
				        }
    });
                   ...
                   .click(() => { // 버튼 클릭시 발생하는 이벤트
                	...
                	// 'currentpage+1'
           	        if(data.hasNext) {
						...
						loadPage(Math.min(currentPage + 1, lastPage),check);
        	        }
    });
    ...
    $("#jump-forward").prop("disabled", data.jumpForwardPage == null || data.jumpForwardPage === currentPage)
                      ...
                      .click(() => {
	               	        if(data.jumpForwardPage != null && data.jumpForwardPage !== currentPage) {
	               	        	loadPage(data.jumpForwardPage, check)
	           		        }
    });
}
