...
const noticeBoardId = Number(pathParts[pathParts.length - 4]);
...
$(document).on("submit", "#post_update_form", function(e) {
    ...
    ajaxWithToken({
		...
		success: function(response) {
		    const isPinned = $('#pin_checkbox').is(':checked');

		    // 기존 값과 다를 때만 핀 API 호출
		    if (response.pinned !== isPinned) {
		        ajaxWithToken({
		            url: `/posts/${postId}/pin`,
		            type: 'PATCH',
		            contentType: 'application/json',
		            data: JSON.stringify({ pinned: isPinned }),
		            success: function() {
		                console.log("핀 설정 반영 완료:", isPinned);
		            },
		            error: function(xhr) {
		    ...
		    alert("게시글이 수정되었습니다.");
		    window.location.href = `/board/${responseBoardId}/notice/${postId}`;
		},
        error: function(xhr) {
