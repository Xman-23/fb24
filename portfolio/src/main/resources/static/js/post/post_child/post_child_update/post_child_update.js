...
const childBoardId = Number(pathParts[pathParts.length - 4]);
const postId = Number(pathParts[pathParts.length - 1]);
const noticeBoard = [1];
const parentBoardIds = [9, 14, 15, 20];
//*****************************************Board ID Start*************************************************************

function check_child_board_number(parentBoardIds) {
    if (!parentBoardIds.includes(childBoardId) && !noticeBoard.includes(childBoardId)) {
		if(token) {
			get_board_api(childBoardId);
		}else {
        ...
        alert("잘못된 접근입니다. 일반 게시판 글작성이 아닙니다.");
		window.location.href="/";
		return;
    }
}
...
$(document).ready(function() {
    check_child_board_number(parentBoardIds);
	image_visible();
