...
public class MemberService {
    ...
    public Member signup(Member member) {
    	...
    	String trimEmail = member.getEmail().trim();
    	String trimPassword = member.getPassword().trim();
    	String trimResidentNumber= member.getResidentNumber().trim();
        ...
        try {
			...
			throw new RuntimeException("주민번호 암호화 실패 : " + e.getStackTrace(), e);
		}
    ...
    public String findEmail(MemberVerifyFindEmailRequestDto verifyFindEmailRequestDto) {
    	...
    	//DTO
    	String dtoUserName = verifyFindEmailRequestDto.getUsername().trim();
    	String dtoResidentNumber  = verifyFindEmailRequestDto.getResidentNumber().trim(); 
    	...
    	List<Member> memberList = memberRepository.findByUsername(dtoUserName);
    	...
    	for(Member member : memberList) {
    		...
    		try{
    			...
    			if(memberUserName.equals(dtoUserName) && decryptResidentNumber.equals(dtoResidentNumber)) {
    				// 임시 JWT 토큰 발급 (dtoUserN
    				return jwtUtil.createTempToken(dtoUserName, dtoResidentNumber);
    			}
    		}catch(Exception e){
    			throw new RuntimeException("복호화 실패 : " + e.getStackTrace(), e);
    		}
    	}
    ...
    public MemberInfoDto getMyInfo(String email){
    	...
    	try {
			...
			throw new RuntimeException("주민번호 복호화 실패 : " + e.getStackTrace(), e);
		}
    ...
    // 회원 정보 변경 Service
    @Transactional //(07.19 02:00 @Transactional 추가)
    public void updateMember(String trimUserName, MemberUpdateRequest dto) {
    	...
    	try {
			...
			throw new DataIntegrityViolationException("회원 정보 변경 DB제약조건 위반으로 인해 save 실패 : " + e.getStackTrace(), e);
		}
    }
