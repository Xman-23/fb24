package com.example.demo.batchscheduler.comment;
...
public class CommentReactionBatchScheduler {
	...
	// 로그
	private static final Logger logger = LoggerFactory.getLogger(CommentReactionBatchScheduler.class);

	//*************************************************** BatchScheduler START ***************************************************//

	// 초(0~59), 분(0~59), 시간(0~23)
	// 일(1~31, 월(1~12) , (0,7(일요일), 1~6(월~토)
	// 요일 중 일요일에 '일' 상관없이 6시0분0초에 배치
	@Scheduled(cron = "0 0 6 ? * 0")
	public void removeReactionsOfDeadtUsers() {

		logger.info("CommentReactionBatchScheduler removeReactionsOfDeadtUsers() Start");
		...
		logger.info("ReactionBatchScheduler removeReactionsOfDeadtUsers() 휴먼 상태인 회원 : {}", deadMember);
		...
		commentReactionService.commentRemoveReactionsByDeadUsers(deadMemberId);

		logger.info("CommentReactionBatchScheduler removeReactionsOfDeadtUsers() End");
	}
