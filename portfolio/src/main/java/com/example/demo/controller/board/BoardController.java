...
public class BoardController {
	...
	public ResponseEntity<?> createBoard(@RequestBody @Valid BoardCreateRequestDTO boardCreateRequestDTO,
		...
		*/
		...
		try {
			...
			// 중복(409)
			ResponseEntity.status(HttpStatus.CONFLICT).body("이미 존재하는 게시판 입니다.");
		}
		logger.info("BoardController createBoard() <========== boardService.createBoard()");
    ...
    public ResponseEntity<?> getAllBoards() {
    	...
    	logger.info("BoardController getAllBoards() Start");

    	logger.info("BoardController getAllBoards() ==========> boardService.getAllBoards()");
    	// 전체 게시판
    	List<BoardResponseDTO> boards  = boardService.getAllBoards();

    	if(!BoardValidation.isValidAllBoard(boards)) {
    		logger.error("BoardController getAllBoards() 조회할 전체 게시판이 존재 하지 않습니다.");
    		return ResponseEntity.status(HttpStatus.NOT_FOUND).body("조회할 전체 게시판이 존재 하지 않습니다.");
    	}
    	logger.info("BoardController getAllBoards() <========== boardService.getAllBoards()");

    	logger.info("BoardController getAllBoards() Success End");
