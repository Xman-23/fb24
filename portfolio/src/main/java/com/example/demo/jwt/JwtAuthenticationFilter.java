...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request,
        ...
        try {
            ...
            if (token != null) {
                ...
                if (jwtUtil.validateToken(token)) {
                    ...
                    Member member = memberRepository.findByEmail(email)
                            .orElseThrow(() -> {
                                logger.error("JwtAuthenticationFilter doFilterInternal findByEmail() Error: 등록된 이메일이 없습니다 email :" + email);
            ...
            logger.error("JwtAuthenticationFilter doFilterInternal IllegalArgumentException Error : 사용자를 찾을 수 없습니다.", e);
            if (!response.isCommitted()) {
                response.sendError(HttpServletResponse.SC_UNAUTHORIZED, "Authentication failed: user not found.");
            }
            return;
        } catch (ExpiredJwtException e) {
            logger.error("JwtAuthenticationFilter doFilterInternal ExpiredJwtException Error : 일반 토큰이 만료 되었습니다.", e);
            if (!response.isCommitted()) {
                response.sendError(HttpServletResponse.SC_UNAUTHORIZED, "Token has expired.");
            }
            return;
        } catch (Exception e) {
            logger.error("JwtAuthenticationFilter doFilterInternal Exception Error : 서버 에러", e);
            if (!response.isCommitted()) {
